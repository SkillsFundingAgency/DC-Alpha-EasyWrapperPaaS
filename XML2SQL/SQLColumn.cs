...
namespace XML2SQL
{
    public class SQLColumn
    {
        ...
        public string SQLDeclaration
        {
            get
            {
                string columnDeclration = DataType.NativeSQLDataType;
                if (DataType.Name == "string" && DataType.Size != 0)
                    columnDeclration += "(" + DataType.Size.ToString() + ")";
                return columnDeclration;
            }
        }
        ...
        public SQLColumn(string name, SQLDataType dataType, bool required, bool isAttribute, SQLTable table)
        {
            Name = name;
            DataType = dataType;
            Required = required;
        ...
        public SQLColumn(string name, XmlSchemaSimpleType schemaType, XmlSchemaObjectTable schemaTypes, bool required, bool isAttribute, SQLTable table)
        {
            Name = name;
            Required = required;
            DataType = GetDataType(schemaType, schemaTypes);
            IsAttribute = isAttribute;
            Table = table;
        }

        private static SQLDataType GetDataType(XmlSchemaSimpleType schemaType, XmlSchemaObjectTable schemaTypes, int size = 0)
        {
            ...
            if (schemaType.Content is XmlSchemaSimpleTypeRestriction)
            {
                ...
                if (restriction.BaseTypeName.Namespace == SQLSchema.NameSpace)
                {
                    ...
                    if (_schemaType is XmlSchemaSimpleType)
                        return GetDataType((XmlSchemaSimpleType)_schemaType, schemaTypes, result.Size);
                    else
            ...
            else if (schemaType.Content is XmlSchemaSimpleTypeUnion)
            {
                ...
                else if (union.BaseTypes != null)
                {
                    if (union.BaseTypes[0] is XmlSchemaSimpleType)
                        return GetDataType((XmlSchemaSimpleType)union.BaseTypes[0], schemaTypes);
                    else
