		...
		<session-data>
			...
			<entity>
				...
				<xsl:for-each select="Learner">
					...
					<xsl:variable name="var_DisadvantageFactor" select="@DisadvantageFactor"/>
					...
					<instance>
						...
						<xsl:if test="string(boolean($var_DisadvantageFactor)) != 'false'">
							<attribute>
								<xsl:attribute name="id">DisadvantageFactor</xsl:attribute>
								<number-val>
									<xsl:value-of select="string($var_DisadvantageFactor)"/>
								</number-val>
							</attribute>
						</xsl:if>
