		...
		<session-data>
			...
			<entity>
				...
				<xsl:for-each select="LearnerEmploymentStatus">
					...
					<xsl:variable name="var_EmpStatMon_RON" select="@EmpStatMon_RON"/>
					...
					<instance>
						...
						<xsl:if test="string(boolean($var_EmpStatMon_RON)) != 'false'">
							<attribute>
								<xsl:attribute name="id">EmpStatMon_RON</xsl:attribute>
								<number-val>
									<xsl:value-of select="string($var_EmpStatMon_RON)"/>
								</number-val>
							</attribute>
						</xsl:if>
