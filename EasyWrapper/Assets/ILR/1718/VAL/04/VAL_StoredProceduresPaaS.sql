				...
				(
					select
						...
						,(
							select
								...
								,(
									select
										...
										(
											...
											from
												...
												left join [$(Postcodes_Schema)].[Postcodes]
										...
										(
											...
											from
												[$(LARS_Schema)].LARS_StandardFunding
											where
										...
										(
											...
											from
												...
												left join [$(EDRS_Schema)].[Employers]
										...
										(
											...
											from
												(
													...
													from
														[$(LARS_Schema)].[LARS_LearningDeliveryCategory_Children]
														inner join [$(LARS_Schema)].[LARS_LearningDeliveryCategory]
									...
									from
										...
										left join [$(LARS_Schema)].[LARS_LearningDelivery] as [LearningDelivery_LARS_LearningDelivery]
											on [LearningDelivery_LARS_LearningDelivery].[LearnAimRef]=[LearningDelivery].[LearnAimRef]
										left join [$(FCT_Schema)].[vw_ContractAllocation]
										...
										left join [$(Org_Schema)].[Org_HMPP_PostCode]
											on [Org_HMPP_PostCode].[HMPPPostCode]=[LearningDelivery].[DelLocPostCode]
										left join [$(LARS_Schema)].[LARS_FrameworkAims]
										...
										left join [$(LARS_Schema)].[LARS_Framework] as [LearningDelivery_LARS_Framework_0_Pathway]
										...
										left join [$(LARS_Schema)].[LARS_Framework] as [LearningDelivery_LARS_Framework]
										...
										left join [$(LARS_Schema)].[LARS_LearningDeliveryCategory_TopMostCategory]
										...
										left join
											(
												...
												from
													[$(LARS_Schema)].[LARS_FrameworkCmnComp]
													inner join [$(LARS_Schema)].[LARS_LearningDelivery]
							...
							from
								...
								left join [$(ULN_Schema)].[UniqueLearnerNumbers2]
					...
					from
						...
						(
							...
							from
								[$(LARS_Schema)].[LARS_Version]
							order by
								MajorNumber desc,
								MinorNumber desc
						) as lv
						cross join [Input].[CollectionDetails] as cd
						cross join [$(Org_Schema)].[Org_Current_Version] as ocv			
						left join
						(
							select distinct
								UKPRN
							...
							from 
								[$(FCT_Schema)].[vw_ContractValidation]
						) as [1618APPS1718]
						...
						(
							...
							from
								[$(FCT_Schema)].vw_ContractValidation as cv
						...
						left join [$(Org_Schema)].[Org_Details]
