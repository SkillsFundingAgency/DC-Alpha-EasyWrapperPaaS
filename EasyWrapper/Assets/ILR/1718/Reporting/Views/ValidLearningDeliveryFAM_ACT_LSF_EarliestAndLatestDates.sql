...
IF OBJECT_ID('[Report].[ValidLearningDeliveryFAM_ACT_LSF_EarliestAndLatestDates]') IS NOT NULL
BEGIN
	DROP VIEW [Report].[ValidLearningDeliveryFAM_ACT_LSF_EarliestAndLatestDates]
END
GO

CREATE VIEW [Report].[ValidLearningDeliveryFAM_ACT_LSF_EarliestAndLatestDates] 
AS
...
SELECT [LearnRefNumber],[AimSeqNumber],[LearnDelFAMType],LearnDelFAMCode,
			LearnDelFamDateFrom_Earliest = ([LearnDelFAMDateFrom]),
			LearnDelFamDateTo_Latest =  ([LearnDelFAMDateTo])
	  FROM [Valid].[LearningDeliveryFAM]
