...
----Main Body	
	...
	SELECT 		
		...
		[Total employer contribution collected (PMR) in previous funding years] = pmrPrev.TNPTotal,
		[Total employer contribution collected (PMR) in this funding year] = pmr.TNPTotal,
		
		[Learning delivery funding and monitoring type – apprenticeship contract type ] = FAM_ACT.LearnDelFAMCode,
		[Learning delivery funding and monitoring type – ACT date applies from (earliest)] = FAM_ACT.LearnDelFamDateFrom_Earliest,
		[Learning delivery funding and monitoring type – ACT date applies to (latest)] = FAM_ACT.LearnDelFamDateTo_Latest,
	...
	LEFT JOIN [Report].[ValidLearningDeliveryFAM_ACT_LSF_EarliestAndLatestDates] FAM_ACT on L.LearnRefNumber = FAM_ACT.LearnRefNumber AND LD.AimSeqNumber = FAM_ACT.AimSeqNumber AND FAM_ACT.LearnDelFAMType = 'ACT' 
	AND AEPE.EpisodeStartDate >= FAM_ACT.LearnDelFamDateFrom_Earliest AND AEPE.EpisodeStartDate <= FAM_ACT.LearnDelFamDateTo_Latest
	LEFT JOIN [Report].[ValidLearningDeliveryFAM_ACT_LSF_EarliestAndLatestDates] FAM_LSF on L.LearnRefNumber = FAM_LSF.LearnRefNumber AND LD.AimSeqNumber = FAM_LSF.AimSeqNumber AND FAM_LSF.LearnDelFAMType = 'LSF'
	...
	) LSF ON LSF.LearnRefNumber = LD.LearnRefNumber AND LSF.AimSeqNumber = LD.AimSeqNumber AND AEPE.EpisodeStartDate = LSF.EpisodeStartDate
