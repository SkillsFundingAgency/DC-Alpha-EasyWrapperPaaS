...
(
	SELECT FundLine, SUM(Total6Months) as Total6Months, SUM(TotalYear) as TotalYear,
			(CASE FundLine
					...
					WHEN 'AEB - Other Learning' THEN 14
					WHEN '19-24 Traineeship' THEN 11
			...
			(CASE FundLine
					...
					WHEN 'AEB - Other Learning' THEN 'AEB - Other Learning - Programme Funding'
		...
		SELECT FundLine, 
			(CASE FundLine 
				...
				WHEN 'AEB - Other Learning' THEN (ISNULL((SELECT [Report].[GetSFAOnProgPayment]('AEB - Other Learning',0)),0) + ISNULL((SELECT [Report].[GetSFAEASForType]('Audit Adjustments: AEB-Other Learning,Authorised Claims: AEB-Other Learning', 0)),0))
					 + ISNULL((SELECT [Report].[GetSFAEmpPayment]('AEB - Other Learning', 0)) ,0) 
					 + ISNULL((SELECT [Report].[GetSFAEFAFundingClaimForType]('25+ Students with an EHCP', 0)) ,0)

				WHEN '19-24 Traineeship' THEN (ISNULL((SELECT [Report].[GetSFAOnProgPayment]('19-24 Traineeship',0)),0) + ISNULL((SELECT [Report].[GetSFAEASForType]('Audit Adjustments: 19-24 Traineeships,Authorised Claims: 19-24 Traineeships', 0)),0))
					+ ISNULL((SELECT [Report].[GetSFAEmpPayment]('19-24 Traineeship', 0)) ,0) + ISNULL((SELECT [Report].[GetSFAEFAFundingClaimForType]('19+ Traineeships (Adult Funded)', 0)),0)
			...
			(CASE FundLine
				...
				WHEN 'AEB - Other Learning' THEN (ISNULL((SELECT [Report].[GetSFAOnProgPayment]('AEB - Other Learning',1)),0) + ISNULL((SELECT [Report].[GetSFAEASForType]('Audit Adjustments: AEB-Other Learning,Authorised Claims: AEB-Other Learning', 1)),0))
					  + ISNULL((SELECT [Report].[GetSFAEmpPayment]('AEB - Other Learning', 1)) ,0)
					  + ISNULL((SELECT [Report].[GetSFAEFAFundingClaimForType]('25+ Students with an EHCP', 1)) ,0)

				WHEN '19-24 Traineeship' THEN (ISNULL((SELECT [Report].[GetSFAOnProgPayment]('19-24 Traineeship',1)),0) + ISNULL((SELECT [Report].[GetSFAEASForType]('Audit Adjustments: 19-24 Traineeships,Authorised Claims: 19-24 Traineeships', 1)),0))
					+ ISNULL((SELECT [Report].[GetSFAEmpPayment]('19-24 Traineeship', 1)) ,0)+ ISNULL((SELECT [Report].[GetSFAEFAFundingClaimForType]('19+ Traineeships (Adult Funded)', 1)),0) 
				...
				WHEN '24+ Trailblazer Apprenticeship' THEN (ISNULL((SELECT [Report].[GetSFATrailblazerProgPayment]('24+ Trailblazer Apprenticeship',1)),0) + ISNULL((SELECT [Report].[GetSFAEASForType]('Audit Adjustments: 24+ Trailblazer Apprenticeships,Authorised Claims: 24+ Trailblazer Apprenticeships', 1)),0)) --Trailblazer code
					...
					END) as TotalYear
				FROM (SELECT stringVal AS FundLine FROM ConvertCSVToTable('19-23 Apprenticeship,24+ Apprenticeship,AEB - Other Learning,19-24 Traineeship,19-23 Trailblazer Apprenticeship,24+ Trailblazer Apprenticeship')) FUNDLINES
