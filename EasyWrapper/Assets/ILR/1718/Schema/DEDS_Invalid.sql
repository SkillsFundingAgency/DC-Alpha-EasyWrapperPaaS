...
as
	select
		...
		,ldh.[HEPostCode]
	from
		Invalid.LearningDeliveryHE as ldh
...
as
	select
		...
		,ldp.[ULN]
	from
		Invalid.LearnerDestinationandProgression as ldp
		cross join (select top 1 UKPRN from Invalid.LearningProvider) as lp
GO
...
as
	select
		...
		,pdm.[ProvSpecDelMon]
	from
		Invalid.ProviderSpecDeliveryMonitoring as pdm
		cross join (select top 1 UKPRN from Invalid.LearningProvider) as lp
GO
...
as
	select
		...
		,ldwp.[WorkPlaceEmpId]
	from
		Invalid.LearningDeliveryWorkPlacement as ldwp
		cross join (select top 1 UKPRN from Invalid.LearningProvider) as lp
GO
...
as
	...
	from
		Invalid.LearningDeliveryFAM as ldf
		cross join (select top 1 UKPRN from Invalid.LearningProvider) as lp
GO
...
as
	select
		...
		,lhfs.[FINAMOUNT]
	from
		Invalid.LearnerHEFinancialSupport as lhfs
		cross join (select top 1 UKPRN from Invalid.LearningProvider) as lp
GO
...
as
	select
		...
		,esm.[ESMCode]
	from
		Invalid.EmploymentStatusMonitoring as esm
		cross join (select top 1 UKPRN from Invalid.LearningProvider) as lp
GO
...
as
	...
	from
		Invalid.ProviderSpecLearnerMonitoring as pslm
		cross join (select top 1 UKPRN from Invalid.LearningProvider) as lp
GO
...
as 
	select
		...
		,lldd.[PrimaryLLDD] 
	from
		Invalid.LLDDandHealthProblem as lldd
		cross join (select top 1 UKPRN from Invalid.LearningProvider) as lp
GO
...
as
	select
		...
		,cp.[ContPrefCode]
	from 
		Invalid.ContactPreference as cp
		cross join (select top 1 UKPRN from Invalid.LearningProvider) as lp
GO
...
as
	...
	from
		Invalid.CollectionDetails as cd
		cross join (select top 1 UKPRN from Invalid.LearningProvider) as lp
GO
