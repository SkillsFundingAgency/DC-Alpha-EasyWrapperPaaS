...
as
	select
		...
		,ldh.[HEPostCode]
	from
		Valid.LearningDeliveryHE as ldh
...
as
	select
		...
		,ldp.[ULN]
	from
		Valid.LearnerDestinationandProgression as ldp
		cross join (select top 1 UKPRN from Valid.LearningProvider) as lp
GO
...
as
	select
		...
		,pdm.[ProvSpecDelMon]
	from
		Valid.ProviderSpecDeliveryMonitoring as pdm
		cross join (select top 1 UKPRN from Valid.LearningProvider) as lp
GO
...
as
	select
		...
		,ldwp.[WorkPlaceEmpId]
	from
		Valid.LearningDeliveryWorkPlacement as ldwp
		cross join (select top 1 UKPRN from Valid.LearningProvider) as lp
GO
...
as
	...
	from
		Valid.LearningDeliveryFAM as ldf
		cross join (select top 1 UKPRN from Valid.LearningProvider) as lp
GO
...
as
	select
		...
		,lhfs.[FINAMOUNT]
	from
		Valid.LearnerHEFinancialSupport as lhfs
		cross join (select top 1 UKPRN from Valid.LearningProvider) as lp
GO
...
as
	select
		...
		,esm.[ESMCode]
	from
		Valid.EmploymentStatusMonitoring as esm
		cross join (select top 1 UKPRN from Valid.LearningProvider) as lp
GO
...
as
	...
	from
		Valid.ProviderSpecLearnerMonitoring as pslm
		cross join (select top 1 UKPRN from Valid.LearningProvider) as lp
GO
...
as 
	select
		...
		,lldd.[PrimaryLLDD] 
	from
		Valid.LLDDandHealthProblem as lldd
		cross join (select top 1 UKPRN from Valid.LearningProvider) as lp
GO
